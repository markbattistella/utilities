	...
	If questionPress = vbYes Then
		...
		FolioPages:=1
 		...
 		If ActiveWorkbook.Saved = False Then
			...
			' Display message if not saved locally
			noSave = MsgBox("You need to save the document before it can export to PDF", vbCritical + vbOKOnly, "Document it not saved")

		Else

			' Set the location and strip extension from original file for PDF export
			originalPath = ActiveDocument.FullName
			filenamePath = Left(originalPath, InStrRev(originalPath, ".") - 1)
			filenamePath = filenamePath & ".pdf"

			ActiveDocument.ExportAsFixedFormat OutputFileName:=filenamePath, _
			...
			' Close the current document after export
